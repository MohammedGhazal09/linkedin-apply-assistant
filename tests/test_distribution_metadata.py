...
def test_python_and_npm_package_identity_stays_synchronized() -> None:
    ...
    assert pyproject_project["version"] == "0.1.5"
...
def test_changelog_tracks_v010_release_without_registry_publication_claims() -> None:
    ...
    assert "## [0.1.5] - 2026-06-13" in changelog
    assert "## [0.1.4] - 2026-06-13" in changelog
...
def test_registry_strategy_keeps_current_version_source_only() -> None:
    ...
    assert "current package metadata version: `0.1.5`" in text
    ...
    assert "npm and powershell update command release uses `0.1.5`" in text
